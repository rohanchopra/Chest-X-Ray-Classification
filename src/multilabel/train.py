...
def train_model(device, model, dataloaders, optimizer, scheduler, criterion, 
                num_epochs, num_classes, is_inception=False, profiler=False, threshold=0.5):
    start = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if (i) % 100 == 0:
                    print(f'Epoch [{epoch + 1}/{num_epochs}], Step [{i}/{total_steps}], Loss: {curr_loss:.4f}, Accuracy: {curr_acc/preds.shape[1]:.2f}%')

            with torch.no_grad():
                epoch_loss = running_loss / len(dataloaders[phase].dataset) 
                epoch_acc = running_corrects.double() / (len(dataloaders[phase].dataset) * num_classes)
